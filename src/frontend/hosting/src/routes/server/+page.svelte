...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-4">
    {#each cards as card}
        <Card padding="xl">
            <h5 class="mb-4 text-xl font-medium text-gray-500 dark:text-gray-400">{card.title}</h5>
            ...
            <ul class="my-7 space-y-4">
                <li class="flex space-x-2 rtl:space-x-reverse">
                    ...
                    <span class="text-base font-normal leading-tight text-gray-500 dark:text-gray-400"> {card.ram} MB RAM </span>
                </li>
                <li class="flex space-x-2 rtl:space-x-reverse">
                    <CheckCircleSolid class="w-4 h-4 text-primary-600 dark:text-primary-500" />
                    <span class="text-base font-normal leading-tight text-gray-500 dark:text-gray-400"> {card.disk} GB Data </span>
                </li>
                <li class="flex space-x-2 rtl:space-x-reverse">
                    <CheckCircleSolid class="w-4 h-4 text-primary-600 dark:text-primary-500" />
                    <span class="text-base font-normal leading-tight text-gray-500 dark:text-gray-400"> {card.cpu} vCPUs</span>
                </li>
            </ul>
            <Button class="w-full" on:click={ () =>  orderServer(card.ID-1) }>Choose Flavour</Button>
        </Card>
    {/each}
</div>
